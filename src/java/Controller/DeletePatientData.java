 ...
 * @author Group 3
 */
@WebServlet(name = "DeletePatientData", urlPatterns =
{
    "/DeletePatientData"
})
public class DeletePatientData extends HttpServlet
{
    ...
    {//gettin parameter id 
        ...
        DAO dao = null;
        String dispatch = "nurseHome.jsp";
        try
        {//handling errors
            if (patientNo == null || patientNo.trim().length() == 0)
            {
            ...
            if (errors.isEmpty())
            {
                ...
                dao = new DAO();
                //deleting 
                boolean isdeleted = dao.deletePatientData(patientNo);
                if (isdeleted)
                {
                    request.setAttribute("Successmsg", "Patient number " + patientNo + " deleted");
                    dispatch = "nurseHome.jsp";
                } else
                {
                    errors.add("Patient not Deleted");

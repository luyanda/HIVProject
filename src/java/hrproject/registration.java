...
public class registration extends HttpServlet
{
    ...
    {//getting patameters from the form
        ...
        {//handling errors for parameters
            ...
            {//setting parameter to Patient
                User p = new User(name, surname, gender, user_age, id, address, cell_num, email, username, password);
                DAO d = new DAO();
                //adding patient       
                boolean isAdded = d.addPatient(p);
                if (isAdded)
                {//go to index page for login
                    request.setAttribute("Successmsg", "Patient " + name + " " + surname + " Registered. Your username is " + username);
                    dispatch = "index.jsp";
                } else
                {//remain on registration page
                    errorlist.add("Patient not registered");
                    request.setAttribute("errorList", errorlist);
                    dispatch = "registerPatient.jsp";
                }
            } else
            {
                request.setAttribute("errorList", errorlist);
